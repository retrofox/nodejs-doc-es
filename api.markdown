...
Un stream es una interface abstracta implementada por varios objetos en Node.
...
Un `Stream de lectura` tiene los siguientes métodos, miembros y eventos.

### Evento: 'data'

`function (data) { }`

El evento `'data'` emite un `Buffer` (predeterminado) o una cadena si fue utilizado
`setEncoding()`.
...
Es emitido cuando el stream ha recibido un EOF (FIN en terminología TCP).
Indica que no habrán más eventos `'data'`. En caso que el stream sea de escritura, 
se podrá continuar escribiendo.

### Evento: 'error'

`function (exception) { }`

Es emitido si hubo un error al recibir datos. 
...
Es emitido cuando el archivo descriptor es recibido en el stream. Solo streams
